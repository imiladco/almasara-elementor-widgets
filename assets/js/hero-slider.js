...
(function () {
	...
	/**
	 * شبکه ایمنیِ فرانت‌اند: اگر افزونه دیگری در همان حلقهٔ
	 * frontend/element_ready خطای uncaught بدهد، هوک‌های صف‌شده بعد از آن —
	 * از جمله همین ویجت — ممکن است اصلاً اجرا نشوند و اسلایدر بالا نیاید.
	 *
	 * عمداً در ویرایشگر خاموش است، نه برعکس: آنجا المنتور با هر رندر دوباره
	 * element_ready را شلیک می‌کند و انتظار زمان‌دارِ کتابخانه هم هست، یعنی
	 * دو مسیر بازیابی دیگر وجود دارد؛ در حالی که DOM مدام تغییر می‌کند و
	 * اسکن مکرر فقط هزینه است. بازدیدکنندهٔ واقعی اما هیچ‌کدام از آن مسیرها
	 * را ندارد، پس محافظ باید دقیقاً همان‌جا روشن باشد.
	 *
	 * دو مهار روی هزینه: فقط وقتی گره‌ای واقعاً اضافه شده باشد (نه صرفِ
	 * حذف)، و حداکثر یک اسکن در هر فریم.
	 */
